...
Future<void> connectDevice(BluetoothDevice device) async {
  if (!canConnectMore) //messaggio errore;
...
\begin{lstlisting}[language=dart, caption={Composizione di tre filtri per la validazione dei dati GPS.}, label={sm_gps_filters}]
  ...
  if (_lastGpsTimestamp != null) {
    final dt = position.timestamp.difference(_lastGpsTimestamp!).inMilliseconds;
    if (dt < MIN_TIME_DELTA_MS) return;
  }
  
  //Filtro 3: Velocita'/accelerazione anormale
...
final smoothedPosition = Position(
  latitude: alpha * position.latitude + (1.0 - alpha) * //ultima latitudine
  longitude: alpha * position.longitude + (1.0 - alpha) * //ultima longitudine
...
\begin{itemize}
  ...
  \item H20: associato ai calzini smart (uguali ai sensori core in aggiunta a 3)
\end{itemize}
...
\end{lstlisting}
Per evitare infine che i layout entrino in collisione tra loro durante le fasi di monitoraggio e di pre-monitoraggio, il sistema utilizza il blocco (\textit{empty-state})all'interno del file HTML. Questo permetterà di informare subito l'utente in caso di assenza di streaming e, successivamente, non appena il file JavaScript otterrà la prima serie di dati valida (\textit{sensors-grid}) l'avviso verrà nascosto autonomamente proseguendo con la normale visualizzazione dei dati. 
