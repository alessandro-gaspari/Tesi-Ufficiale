...
Per evitare il passaggio di numerose quantità di parametri nel codice, in Flutter si utilizzano i provider. Essi agiscono come dei contenitori di dati, che alla variazione di essi, invocheranno il metodo \textit{NotifyListeners} avvisando autonomamente tutti i widget che si sono sottoscritti ad una specifica classe. In tal modo si ricostruiranno solo le porzioni di schermo interessate al cambiamento, mantenendo così fluidità e prestazioni elevate.
...
\begin{enumerate}
  ...
  \item Il profilo da cancellare deve essere quello effettivamente selezionato, se sarà così allora verra selezionato come attivo il più vicino ad esso nella lista dei profili e poi il sistema procederà all'eliminazione del desiderato.
\end{enumerate}
...
Per poter correttamente inviare i dati provenienti dai sensori alla dashboard salvandoli su un server remoto, l'applicazione necessita di ulteriori file indispensabili. Uno di essi prende il nome di \textit{tcp client} e implementa per l'appunto, un canale TCP\footnote{Il Transimission Control Protocol (TCP) è un protocollo di rete che assicura scambi di dati affidabili, ordinati e integri tra dispositivi sulla rete.} persistente in grado di gestire grandi quantità di dati ad alte frequenze senza delay o overhead eccessivi. All'interno di essa si trova la classe \textit{TCPDataSender} che si occuperà di collegarsi al server remoto gestendo autonomamente disconessioni e riconessioni. Il primo metodo al suo interno si chiama \textit{connect} e prevede la connessione con il server remoto tramite handshake con timeout di tre secondi. Tramite dei controlli \textit{if} il sistema impedirà all'applicazione di creare più tentativi di connesioni in contemporanea, il che potrebbe causare perdite di memoria o altri errori. Una volta stabilita la connessione, per rilevare eventuali chiusure forzate o perdite di pacchetti, il sistema si iscriverà allo stream di eventi \textit{done} o \textit{catchError}.
...
\end{lstlisting}
Per evitare che il dispositivo su cui viene eseguita l'applicazione interrompa le streaming dati a schermo spento o dopo qualche minuto di inattività, all'interno dello \textit{streaming manager} viene implementato un meccanismo che impedisce al processore di entrare in modalità di risparmio energetico, nominato \textit{WakelockPlus}. Successivamente verrà anche connesso il \textit{TCPDataSender} per poter così aprire il canale verso il server e avviare gli streaming. 
Per ovviare il problema della generazione frequente di errori da parte del GPS dello smartphone come accuratezza insufficiente, salti di posizione o dati duplicati, nel sistema è stato implementata una combinazione di filtri ispirata al filtro di Kalman\footnote{Il filtro di Kalman è un algoritmo matematico che stima degli stati reali, unendo previsioni teoriche con misure dei sensori, riducendo errori e rumori.}.
...
\]
Al suo interno si troveranno i seguenti dati:ù
\begin{itemize}
